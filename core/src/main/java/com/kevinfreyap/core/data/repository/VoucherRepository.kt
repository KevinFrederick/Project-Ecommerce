    ...
    override suspend fun syncVouchers() = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val publicSnapshot = firestore.collection(VOUCHER_COLLECTION)
                ...
                .get()
                .await()
            val publicList = publicSnapshot.toObjects(Voucher::class.java)

            // Fetch Personal
            val personalSnapshot = firestore.collection(USER_COLLECTION)
                .document(uid)
                .collection(VOUCHER_COLLECTION)
                .get()
                .await()
            val personalList = personalSnapshot.toObjects(Voucher::class.java)
            ...
            val existingIds = voucherDao.getAllIds().toSet()

            val existingStatusMap = voucherDao.getIdsAndNewStatus()

            val entities = voucherMap.values.map { domainModel ->
                var entity = DataMapper.mapVoucherDomainToVoucherEntity(domainModel)

                if (!existingIds.contains(domainModel.id)) {
                    entity = entity.copy(isNew = true)
                } else {
                    val currentStatus = existingStatusMap[domainModel.id] ?: false
                    entity = entity.copy(isNew = currentStatus)
                }
